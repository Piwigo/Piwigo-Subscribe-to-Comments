...
<p>
  {if !empty($STC.element)}<a href="{$STC.element.url}" title="{$STC.element.name}">{'Return to item page'|translate}</a><br>{/if}
  <a href="{$U_MANAGE_SUBSCRIPTIONS}">{'Manage my subscriptions'|translate}</a>
</p>

{else}
<form action="{$U_MANAGE_SUBSCRIPTIONS}" method="post">
    ...
    <table class="subscriptions_list">
      ...
      <tr class="{if $smarty.foreach.subs_loop.index is odd}row1{else}row2{/if}">
        ...
        <td style="white-space:nowrap;">
          <div class="actions">
            <a href="{$sub.U_UNSUB}" class="unsub">{'Unsubscribe'|translate}</a>
            {if $sub.U_VALIDATE}<br> <a href="{$sub.U_VALIDATE}">{'Validate'|translate}</a>{/if}
          </div>
        </td>
        <td style="white-space:nowrap;">
